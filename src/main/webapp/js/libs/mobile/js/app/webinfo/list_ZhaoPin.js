...
$('#section_container').on('articleshow','#zhaopinxinxi',function(){
  ...
  var asides = J.Page.loadContent('webinfo/typeSelect.html?num=2');
  var $asides = $(asides);
  $('#aside_container').append($asides);
	...
	function refresh_zhaopin(scroll){
		...
		if(scroll==undefined){
			 $.ajax({
		         url :ctx+"service/getTestCabinet",
		         timeout : 9000,
		       	...
		       	var html = '';
			            for(var i=0;i<msg.data.length;i++){
			                html += '<li data-icon="next" data-selected="selected">'+
			                	'<a href="'+ctx+'modile/webinfo/ZhaoPinItem.html?id='+msg.data[i].id+'" class="one-part"  data-target="link">'+
			                '<div >'+
		 	            msg.data[i].name+
		 	            '</div>'+
		 	            '</a></li>'
