...
$('#section_container').on('articleshow','#zhaopinxinxi',function(){
  ...
  var asides = J.Page.loadContent('webinfo/typeSelect.html?num=4');
  var $asides = $(asides);
		       	...
		       	var html = '';
			            for(var i=0;i<msg.data.length;i++){
			                html += '<li data-icon="next" data-selected="selected">'+
			                	'<a href="'+ctx+'modile/webinfo/ershouxinxiItem.html?id='+msg.data[i].id+'" class="one-part"  data-target="link">'+
			                '<div >'+
		 	            msg.data[i].title+
		 	            '</div>'+
		 	            '</a></li>'
