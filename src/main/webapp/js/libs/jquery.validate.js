...
$.extend($.validator, {
	...
	methods: {
		...
		differentFrom : function(value, element, param) {
		...
		validatPwd : function(value, element) {
			var validate=true;
			var regis=new RegExp("(?![a-z]+$|[0-9]+$|_+$)^[a-zA-Z0-9_]{6,}$");//只能为字母和数字或下划线中的两个的组合
//			if(/^(\d)\1{5,31}$/.test(value)){//不能为相同数字
			if(regis.test(value)){
				validate=true;
			}else{
				validate=false;
