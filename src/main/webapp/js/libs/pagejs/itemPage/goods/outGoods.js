...
var inOrder_query_chance_name="";
var inOrder_query_chance_id="";
$(function () {
...
function inorder_query_name(page){
	...
	var chanceid=$("#inOrder_queryData_Body_query_chance_id").val();
	if(chanceid==""){
		chanceid="0";
	}
...
function inGoods_box_save(type){
	...
	if(data2!=""){
		...
		var chanceid=$("#inOrder_queryData_Body_add_id").val();
		if(chanceid==""){
			$("#inOrder_queryData_Body_add_name").val("请选择客户");
			return ;
		}
		if(name==""||name=="请填写计划名称"){
			...
			$.ajax({
				type : "POST",
				url : ctx+"goods/function/createOrder",
				data:{
					name:name,
					data:data2,
					chanceid:chanceid,
...
function inGoods_changeNum_input(obj){
	...
	var inprice="";
	var num="";
	if($("#"+lineNum+"_3_input").val()==undefined){
		inprice=$("#"+lineNum+"_3").html();
		num=$("#"+lineNum+"_11_input").val()
	}else{
		inprice=$("#"+lineNum+"_3_input").val();
...
function inOrder_box_look(id){
	...
	inOrder_query_chance_name=$("#inOrder_queryData_Body_query_chance_name").val();
	inOrder_query_chance_id=$("#inOrder_queryData_Body_query_chance_id").val();
	var i=$.layer({
	    type : 3
	});
	$.ajax({
		...
		success : function(msg) {
			...
			if(msg.success){
				...
				for(var ii=0;ii<msg.data.length;ii++){
					countPrice=countPrice+msg.data[ii].goodsNum*msg.data[ii].price;
				}
				$("#inOrder_box_name").val(msg.obj.title);
				$("#orderNum").html(msg.obj.orderNum);
				$("#orderNum_id").html(msg.obj.id);
				$("#inOrder_countPrice").val(countPrice);
				$("#inOrder_queryData_Body_add_name").val(msg.obj.linkmanName);
				$("#inOrder_queryData_Body_add_id").val(msg.obj.linkmanId);
			}else{
				alertError(msg.info,"alertDiv");
			}
		}
...
function inGoods_box_look_return(){
	...
	$("#inOrder_queryData_Body_query_chance_name").val(inOrder_query_chance_name);
	$("#inOrder_queryData_Body_query_chance_id").val(inOrder_query_chance_id);
	if(inOrder_query_type=="num"){
		inorder_query_num();
	}else{
		inorder_query_name();
...
function inGoods_select_chance(elementId,elementId2){
	var i2 = $.layer({
		type : 3
	});
	$.ajax({
		...
		success : function(msg) {
			...
			if (msg.success == null || msg.success == undefined || msg.success == true) {
				...
				Edit_Item_ElementId_id = elementId2;
				Edit_Item_ElementId_text = elementId;
				Edit_AddLinkManWindow_Query_alone_linkman=true;
