...
@Controller
@RequestMapping(value = "/ImageModelController")
public class ImageController {
	...
	public Map getGonggao(Model model, HttpServletRequest req,
		...
		if(type==null){
			...
			List<Object> list=dao.getObjectListPage("VipImageMessage", "where conpanyId="+users.getConpanyId()+" and pass=0 order by createDate desc",Integer.parseInt(nowpage),Integer.parseInt(countNum));
			map.put("success", true);
			map.put("data", list);
		}else if(type.equals("pass")){
			String id=req.getParameter("id");
			List<Object> list=dao.getObjectList("VipImageMessage", "where id="+id+" and conpanyId="+users.getConpanyId()+" order by createDate desc");
			if(list.size()>0){
				VipImageMessage mes=(VipImageMessage) list.iterator().next();
				mes.setPass(1);
				dao.update(mes);
				map.put("success", true);
				map.put("info", "状态修改成功");
